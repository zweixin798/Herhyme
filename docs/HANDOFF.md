...
- **记录**：只展示周/月/年身体地图、近 7 天五类信号趋势、恢复状态、完整度和确定性建议，不展示自然语言输入或原文记录。
- **快速记录**：从今日页进入独立非 Tab 页面，完成自然语言解析、识别结果确认和修改；确认后才进入身体地图。
...
| `pages/checkin/` | 自然语言解析、人工确认和归档 |
| `pages/record/` | 身体地图、近期趋势、恢复状态和确定性建议 |
...
记录页恢复状态同样使用确定性规则，并固定读取近 7 天：

- 睡眠有效记录均值低于 6 小时，或疲惫、压力、低落信号达到 2 次时，提示放慢节奏。
- 近 1 天有训练时提示身体处于恢复中；近 2 天力量训练可给出轻量活动和温和拉伸建议。
- 疼痛程度达到 7/10 时优先提示降低负担，并保留寻求专业帮助的安全边界。
- 缺失的睡眠或训练时长显示为“待记录/还缺少”，绝不按 0 参与计算。
- 数据不足时显示“形成中”，不会宣称身体状态稳定。
